...
export function SearchProvider({ children }: { children: ReactNode }) {
    ...
    // Limpa estado de busca ao sair da página inicial
    useEffect(() => {
        if (pathname !== '/') {
    ...
    // Busca fotos aleatórias se o usuário ainda não pesquisou nada
    useEffect(() => {
        if (!hasSearched) {
            fetchRandomPhotos()
                .then(data => {
    ...
    // Função para buscar fotos com base na palavra-chave
